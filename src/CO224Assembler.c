 	...
 	while(fgets(line, LINE_SIZE, fi)!=NULL) // Read a line from the input .s file
	{
		...
		*******************************************************************/
		...
		while(in_token!=NULL)
		{
			...
			else if(strcasecmp(in_token,"bnq")==0) strcpy(out_token, op_bnq);
			else if(strcasecmp(in_token,"sll")==0) strcpy(out_token, op_mult);
			else if(strcasecmp(in_token,"srl")==0) strcpy(out_token, op_sll);
